...
def batch_learning_svm(task, augment_token_selection_strategy, instance_augmentation_strategy, fine_tune_model, augment_type, roberta_model, tokenizer):
    ...
    dataset = pd.read_csv(augment_data_file, usecols=['full_text', task, 'is_test', 'is_origin'])
    # print(dataset.columns)

    train_dataset = dataset.loc[dataset['is_test'] == 0]
    test_dataset = dataset.loc[(dataset['is_test'] == 1) & (dataset['is_origin'] == 1)]
    ...
    #     print("epoch number: {}".format(epoch))
    if task != 'priority':
        for batch_features, batch_labels in batch_generator_get_features(x_train, y_train, batch_size, roberta_model, tokenizer, features_file_path):
            model.partial_fit(batch_features, batch_labels, classes=[0, 1])
    else:
        for batch_features, batch_labels in batch_generator_get_features(x_train, y_train, batch_size, roberta_model, tokenizer, features_file_path):
            model.partial_fit(batch_features, batch_labels, classes=[0, 1, 2])
    ...
    # print("F-score (micro): {}".format(f1_micro))
...
if __name__ == '__main__':
    ...
    # task_names = ['reassignment']
    # task_names = ['priority', 'reopen', 'severity']
    # model_types = ['full_text_fine_tune']
    model_types = [
        'full_text_fine_tune',
        'key_sentences_fine_tune_at_least_2_context_2_exception',
    ...
    # model_types = [
    #     # 'full_text_fine_tune',
    #     'key_sentences_fine_tune_top_1_context_1_exception',
    #     'key_sentences_fine_tune_top_1_context_1_template',
    #     'key_sentences_fine_tune_top_1_context_1_remove',
    #     'key_sentences_fine_tune_top_1_context_1',
    #     'key_sentences_fine_tune_top_1',
    #     'key_sentences_fine_tune_at_least_1_context_1_exception',
    #     'key_sentences_fine_tune_at_least_1_context_1_template',
    #     'key_sentences_fine_tune_at_least_1_context_1_remove',
    #     'key_sentences_fine_tune_at_least_1_context_1',
    #     'key_sentences_fine_tune_at_least_1',
    # ]
    ...
    for model_type in model_types:
        ...
        if model_type.startswith('key_sentences_fine_tune_'):
            model_name = model_type.replace('key_sentences_fine_tune_', '')
            # 句子选择类型
            sentence_selection = 'top-k' if model_name.startswith('top_') else 'at least'
            # K值, 是否获取上下文, 上文数量, 下文数量, 堆栈信息
            model_name = model_name.replace('top_', '') if sentence_selection == 'top-k' else model_name.replace(
                'at_least_', '')
            ...
            if idx == -1:
                ...
                k_value = model_name[: idx]
                elements = model_name[idx + 1:].split('_')
                get_context = '是'
                context_before_num = elements[1]
                context_after_num = elements[1]
                if len(elements) == 2:
                    stack_trace = ''
                else:
                    stack_trace = elements[2]
        else:
            sentence_selection = ''
            k_value = ''
            get_context = ''
        ...
        for augment_tool_type in augment_tool_types:
            for task_name in task_names:
                ...
                if task_name == 'reopen' or task_name == 'priority':
                    # f1, precision, recall = batch_learning_svm(task_name, 'text', 'least', model_type, augment_tool_type, roberta, roberta_tokenizer)
                    # single_data = [sentence_selection, k_value, get_context, context_before_num, context_after_num,
                    #                stack_trace, augment_tool_type, 'text', 'least', f1, precision, recall]
                    # data.append(single_data)

                    f1, precision, recall = batch_learning_svm(task_name, 'sentence', 'least', model_type, augment_tool_type, roberta, roberta_tokenizer)
                    # single_data = [sentence_selection, k_value, get_context, context_before_num, context_after_num,
                    #                stack_trace, augment_tool_type, 'sentence', 'least', f1, precision, recall]
                    # data.append(single_data)
                    # res = pd.DataFrame(data)
                    # res.to_csv(task_name + '_result_records_full.csv', index=False, header=False, mode='a')
                ...
                # batch_learning_svm(task_name, 'sentence', 'least', 'full_text_fine_tune', augment_tool_type)
                    ...
                    for token_strategy in token_strategies:
                        for instance_strategy in instance_strategies:
                            f1, precision, recall = batch_learning_svm(task_name, token_strategy, instance_strategy, model_type, augment_tool_type, roberta, roberta_tokenizer)
                            single_data = [sentence_selection, k_value, get_context, context_before_num,
                                           context_after_num, stack_trace, augment_tool_type, token_strategy,
                                           instance_strategy, f1, precision, recall]
                            data.append(single_data)
